...
def tokenize(document):
    ...
    final_list = [
        x for x in tokenized if x not in string.punctuation and x not in nltk.corpus.stopwords.words("english")]
...
def top_sentences(query, sentences, idfs, n):
    ...
    sorted_by_score = [k for k, v in sorted(
        scores.items(), key=lambda x: (x[1][0], x[1][1]), reverse=True)]
